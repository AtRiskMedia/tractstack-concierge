...
function neo4j_getUserGraph($client, $conn, $visit_id)
{
  ...
  if ($neo4j_visit) {
    ...
    $results = $client->writeTransaction(static function (TransactionInterface $tsx) use ($neo4j_visit) {
      $results = $tsx->run('MATCH (f:Fingerprint)-[cc]-(v:Visit) WHERE ID(v)=$neo4j_visit
        OPTIONAL MATCH (v:Visit)-[r]-(c) 
        OPTIONAL MATCH (b:Belief)-[bb]-(f)
        OPTIONAL MATCH (c)<-[a:CONTAINS]-(s)<-[d:CONTAINS]-(t) RETURN *', [
